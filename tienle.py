...
app = Flask(__name__)
app.secret_key = 'bda7159bfbaf4a003117ec3c70e6b5b5b0dd90ff3aaf7d1853e26048f2639b27e26ae12d2a4d5ff746d63f3a97bcf3c6761d4edcfecc0fc69ef7cb9de356d997'  # Change this to a strong secret key
...
@app.route('/loaderio-c6a9641d263888e89c1ba7189b12ab41/')
def verify_loaderio():
    return 'loaderio-c6a9641d263888e89c1ba7189b12ab41'
...
def get_client_ip():
    """Return client IP considering proxy headers."""
    ...
    if xff:
        ...
        ip = request.headers.get('X-Real-IP', request.remote_addr or request.environ.get('REMOTE_ADDR', ''))
    if ip == '::1':
        ip = '127.0.0.1'
    return ip
...
def log_access(ip, user_agent, action, is_admin=False, filename=None):
    """Log access with geo information (use proxy headers if present)."""
    try:
        from flask import request
        # Prefer headers / computed client IP
        client_ip = get_client_ip() or ip or '127.0.0.1'

        # If IP is private/loopback, skip external geo lookup
        geo_info = {}
        if is_private_ip(client_ip):
            geo_info = {
                "status": "private",
                "message": "local or private IP",
                "query": client_ip
            }
        else:
            try:
                response = requests.get(f"http://ip-api.com/json/{client_ip}?fields=status,message,country,regionName,city,isp,query", timeout=5)
